    ...
    <div class="bg-white shadow rounded-lg p-6">
        ...
        <div class="">
            <div class="overflow-x-auto">
                <!-- Recipe Image -->
                <div class="my-2">
                    <h2 class="py-2 text-lg">Image :</h2>
                    <img class="rounded-lg w-[650px] h-[380px]"
                        @if ($recipe->photo) src="{{ asset($recipe->photo) }}" @else src="{{ asset('uploads/no-image.png') }}" @endif>
                </div>

                <table class="min-w-full border border-gray-300 divide-y divide-gray-500">
                    <thead class="">
                        {{-- <td>
                            <!-- Recipe Image -->
                            <div class="recipe-image py-2 px-2 flex justify-items-center">
                                <img class="rounded-lg w-[850px] h-[380px]"
                                    @if ($recipe->photo) src="{{ asset($recipe->photo) }}" @else src="{{ asset('uploads/no-image.png') }}" @endif> 
                            </div>
                        </td> --}}
                        ...
                        <tr class="border-b-2 ">
                            <th
                                ...
                                <span class="py-1">Cook Time :</span>
                            </th>
                            <td>
                                <div class="flex gap-3">
                                    {{ $recipe->cook_time }} <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                        ...
                        <tr class=" border-b-2 ">
                            <th class="py-6 px-2 text-left text-xs font-medium text-gray-700 uppercase">
                                Author : </th>
                            <td><span class="rounded px-2 py-1 text-white bg-blue-500">{{ $recipe->user->name }}</span>
                            </td>
                        </tr>

                        <tr class="bg-gray-200 border-b-2 divide-red-50 ">
                            <th class="py-6 px-2 text-left text-xs font-medium text-gray-700 uppercase">
                                Nutrition Text : </th>
                        ...
                        <tr class="bg-gray-200">
                            <th class="py-6 px-2 text-left text-xs font-medium text-gray-700 uppercase">
                                Directions : </th>
                            <td class="py-3">{!! $recipe->directions !!}</td>
                        </tr>
                        <!-- Ingredients -->
                        <tr class="mb-3">
                            <th class="py-6 px-2 text-left text-xs font-medium text-gray-700 uppercase">
                            ...
                            <td>
                                @foreach ($recipe->ingredients as $ingredient)
                                    <div class="px-2 py-3 mt-2 flex flex-col gap-2">
                                        <span
                                    ...
                                    @foreach (json_decode($ingredient['ingredients_list'], true) as $item)
                                        {{-- <li>{{ $item }}</li> --}}
                                        <span
                                            class="bg-indigo-100 text-indigo-800 text-xs font-medium me-2 px-3 py-2 rounded dark:bg-gray-700 dark:text-indigo-400 border border-indigo-400">{{ $item }}</span>
                                    @endforeach
                                @endforeach
                            </td>
