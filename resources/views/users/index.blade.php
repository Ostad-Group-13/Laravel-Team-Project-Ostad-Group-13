            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <form action="{{ route('users.destroy', $user->id) }}" method="post">
                            ...
                            @if (in_array('Super Admin', $user->getRoleNames()->toArray() ?? []) )
                                ...
                                @if (Auth::user()->hasRole('Super Admin') && Auth::user()->id == $user->id)
                                    ...
                                    <x-danger-button type="submit" onclick="return confirm('Do you want to delete this user?');"><i class="bi bi-trash"></i> Delete</x-danger-button>
                                @endif
                            @else
                                @can('edit-user')
                                    <a href="{{ route('users.edit', $user->id) }}" class="btn btn-primary btn-sm"><i class="bi bi-pencil-square"></i> Edit</a>
                        ...
                        {{-- <form action="{{ route('roles.destroy', $user->id, "Delete-Role") }}" method="post">
                            @csrf
                            @method('DELETE')
                            @if (in_array('Super Admin', $user->getRoleNames()->toArray() ?? []) )
                                @if (Auth::user()->hasRole('Super Admin'))
                            ...
                            @else
                                @can('delete-user' || 'edit-user')
                                    @if (Auth::user()->id!=$user->id)
                                        <button type="submit" onclick="return confirm('Do you want to Clear roles of this user?');"><i class="bi bi-trash"></i> Clear</button>
                                    @endif
                                @endcan
                            @endif
                        </form> --}}
