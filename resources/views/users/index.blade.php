                        ...
                        <tr>
                            ...
                            <td class="px-4 py-2 text-sm text-gray-700 space-x-2">
                                <form action="{{ route('users.destroy', $user->id) }}" method="post" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <a href="{{ route('users.show', $user->id) }}" class="show-btn inline-flex items-center gap-2">
                                        <i class="bi bi-eye"></i> Show
                                    </a>
                                    @if (in_array('Super Admin', $user->getRoleNames()->toArray() ?? []))
                                        @if (Auth::user()->hasRole('Super Admin'))
                                            <a href="{{ route('users.edit', $user->id) }}" class="edit-btn inline-flex items-center gap-2">
                                                <i class="bi bi-pencil-square"></i> Edit
                                            </a>
                                        @endif
                                        @if (Auth::user()->hasRole('Super Admin') && Auth::user()->id == $user->id)
                                            <button type="submit" class="delete-btn inline-flex items-center gap-2" onclick="return confirm('Do you want to delete this user?');">
