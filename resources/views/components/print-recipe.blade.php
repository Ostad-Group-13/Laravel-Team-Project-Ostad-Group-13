    ...
    <div class="container">
        <div class="racipe_details_wrapp flex flex-col lg:flex-row gap-[40px] items-center">
            <div class="rd_head_info w-full">
                <h1 class="xl:text-[64px] md:text-[50px] text-[35px] leading-tight font-semibold text-black">
                    {{ $recipe->title }}</h1>
                    ...
                    <div class="rd_head_info_item">
                        <div class="flex items-center gap-[16px]">
                            <img class="w-[50px] h-[50px] rounded-full"
                                src="{{ asset($recipe->user->profile_photo_url) }}" alt="user profile image">
                            <div class="flex flex-col">
                                <span class="font-medium text-sm">{{ $recipe->user->name }}</span>
                                @if ($recipe->created_at == '')
                                    <span>no</span>
                                @else
                                    <p class="text-[#00000099] text-sm font-medium">
                                        {{ $recipe->created_at->format('d F Y') }}</p>
                                @endif
                            </div>
                        </div>
                    </div>
                    <div class="rd_head_info_item">
                        <div class="flex items-center sm:justify-center justify-start gap-[10px]">
                            ...
                            <div class="flex flex-col">
                                ...
                                <p class="text-[#00000099] text-sm font-medium">{{ $recipe->pre_time }}
                                    {{ 'Minutes' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <ul class="mt-[30px]">
                            @foreach ($recipe->nutritions as $nutrition)
                                <li><span>{{ $nutrition->name }}</span> <span class="nu_value">{{ $nutrition->amount }}
                                        {{ $nutrition->unit }}</span></li>
                            @endforeach
                        </ul>
                    </div>
                                ...
                                @foreach (json_decode($ingredient['ingredients_list'], true) as $item)
                                    <li>
                                        <label class="flex items-center space-x-2">
                                            ...
                                            <span
                                                class="w-6 h-6 ml-0 flex rounded-full border-2 border-gray-300 items-center justify-center peer-checked:border-black peer-checked:bg-black">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                    viewBox="0 0 24 24" stroke-width="2" stroke="white"
                                                    class="w-4 h-4 peer-checked:block">
